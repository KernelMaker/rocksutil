...
Status NewRandomAccessFile(Env* env, const std::string& fname,
                       unique_ptr<RandomAccessFile>* result,
                       const EnvOptions& options) {
  Status s = env->NewRandomAccessFile(fname, result, options);
  return s;
}

Status NewSequentialFile(Env* env, const std::string& fname,
                       unique_ptr<SequentialFile>* result,
                       const EnvOptions& options) {
  Status s = env->NewSequentialFile(fname, result, options);
  return s;
}
